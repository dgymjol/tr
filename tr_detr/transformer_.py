...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_queries=2, num_encoder_layers=6,
        ...
        if tome_cross:
            t2v_encoder_layer = T2V_TransformerEncoderLayer_TOME(d_model, nhead, dim_feedforward,
                                                    dropout, activation, normalize_before)
            encoder_norm = nn.LayerNorm(d_model) if normalize_before else None
            fc_norm = nn.LayerNorm(d_model) if tome_pool else None
            self.t2v_encoder = TransformerEncoder_TOME(t2v_encoder_layer, num_encoder_layers, encoder_norm,
        ...
        if tome:
            ...
            # decoder_layer = TransformerDecoderLayer_TOME(d_model, nhead, dim_feedforward,
            #                                         dropout, activation, normalize_before, keep_query_pos=keep_query_pos)
            # decoder_norm = nn.LayerNorm(d_model)
            # self.decoder = TransformerDecoder_TOME(decoder_layer, num_decoder_layers, decoder_norm,
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, src,
        ...
        if not return_all:
            return self._forward(src, mask, src_key_padding_mask, pos, **kwargs)
            
        else:
            return self._forward(src, mask, src_key_padding_mask, pos, **kwargs), pos, src_key_padding_mask

class TransformerEncoder_TOME(nn.Module):

    def __init__(self, encoder_layer, num_layers, norm=None, return_intermediate=False,
                 tome_pool=False, fc_norm=None):
        super().__init__()
        self.layers = _get_clones(encoder_layer, num_layers)
        self.num_layers = num_layers
        self.norm = norm
        self.return_intermediate = return_intermediate
        self.tome_pool = tome_pool
        if tome_pool:
    ...
    def forward_normal(self, src,
        ...
        for layer in self.layers:
            output, _, _, _ = layer(output, tome=False, src_key_padding_mask=src_key_padding_mask, 
                           pos=pos,  **kwargs)
            if self.return_intermediate:
                intermediate.append(output)
    ...
    def forward_tome(self, src,
        ...
        for layer in self.layers:
            output, size, pos, src_key_padding_mask = layer(output, tome=True, tome_size=self.tome_size,
                           src_key_padding_mask=src_key_padding_mask, pos=pos, **kwargs)
            self.tome_size = size
            if self.return_intermediate:
                intermediate.append(output)
        ...
        if self.tome_pool:
            ...
            if self.tome_size is not None:
                output = (output * size.transpose(0, 1)).sum(dim=0) / seqlen
            else:
                output = output.mean(dim=0)  # global pool without cls token
            # ---- End of change ----
    ...
    def forward(self, src,
        ...
        if not return_all:
            return self.forward_normal(src, mask, src_key_padding_mask, pos, **kwargs)
        else:
            return self.forward_tome(src, mask, src_key_padding_mask, pos, **kwargs)
...
class TransformerDecoder_TOME(nn.Module):
    ...
    def forward(self, tgt, memory,
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            output = layer(output, memory, tgt_mask=tgt_mask,
                           ...
                           memory_key_padding_mask=memory_key_padding_mask,
                           pos=pos, query_pos=query_pos, query_sine_embed=query_sine_embed,
                           is_first=(layer_id == 0), tome_size=tome_size)
...
class TransformerEncoderLayer_TOME(nn.Module):

    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
                 activation="relu", normalize_before=False, r=0):
        super().__init__()
        
        self.self_attn = ToMeSelfAttention(d_model, nhead, attn_drop=dropout)
    ...
    def forward(self,
        ...
        x = x.transpose(0, 1) # (bs, seq_len, dim)
        
        if not tome:
            src2, metric = self.self_attn(x, key_padding_mask=src_key_padding_mask)   
            src = src + self.dropout1(src2.transpose(0, 1)) #(seq_len, bs, dim)
            
        else:
            src2, metric = self.self_attn(x, tome_size,
                        key_padding_mask=src_key_padding_mask)
            src = src.transpose(0, 1) + self.dropout1(src2) #(bs, seq_len, dim)
            
            # Merging
            if tome_size is None:
                tome_size = torch.ones_like(src[..., 0, None])
            src, tome_size, pos_, src_key_padding_mask = adjacent_matching(src, metric, self.r, tome_size, pos.transpose(0, 1), src_key_padding_mask.unsqueeze(-1))
            # src, tome_size, pos_, src_key_padding_mask = bipartite_matching(src, metric, self.r, tome_size, pos.transpose(0, 1), src_key_padding_mask.unsqueeze(-1))
            
            src = src.transpose(0, 1) #(seq_len, bs, dim)
            pos = pos_.transpose(0, 1) #(seq_len, bs, dim)
        ...
        src = src + self.dropout2(src2)
        src = self.norm2(src)
        return src, tome_size, pos, src_key_padding_mask
...
class TransformerDecoderLayer_TOME(nn.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
        ...
        self.ca_qpos_sine_proj = nn.Linear(d_model, d_model)
        # self.cross_attn = MultiheadAttention(d_model * 2, nhead, dropout=dropout, vdim=d_model)
        self.cross_attn = ToMeCrossAttention(d_model * 2, d_model, nhead, attn_drop=dropout)
